...
public class TrafficLight {
	...
	/**
	 * On instantiation, generates a random number between 10 and 50. This is
	 * the number of ticks the light will take before turning the red light on.
	 * It is meant to represent the fact that some traffic lights stay on green
	 * light for longer than others.
	 */
	private int noLightLength = ThreadLocalRandom.current().nextInt(10, 50 + 1);

	/**
	 * The number of ticks the red light has been on for in the most recent
	 * cycle (will reset when redLightLength is reached).
	 */
	private int numOfRedTicks;

	/**
	 * Stores the number of ticks the light has been off for in the most recent
	 * cycle (will reset when noLightLength is reached).
	 */
	...
	TrafficLight(int y, int x, int redLightLength) {
		...
		// Subtracts 100 from the tile the light is on to turn the light off as
		// the default stance.
	...
	public void tick() {
		...
		if (!redLight) {
			...
			if (numOfOffTicks == noLightLength) {
				...
				redLight = true;
				// Updates the temporary map in Storage by adding 100 to the
				// position of
				// this traffic light.
				double[][] map = Storage.getInstance().getTempMap();
			...
			if (numOfRedTicks == redLightLength) {
				...
				redLight = false;
				// Updates the temporary map in Storage by subtracting 100 to
				// the
	...
	public void resetLight() {
		...
		// If the traffic light is currently off and there is no vehicle on the
		// tile.
		if (map[yPosition][xPosition] > 0 && map[yPosition][xPosition] <= 4) {
			map[yPosition][xPosition] = map[yPosition][xPosition] + 100;
		}
		// If the traffic light is currently off and there is a vehicle on the
		// tile (takes into account the 9 value of the vehicle).
		else if (map[yPosition][xPosition] >= 10
				&& map[yPosition][xPosition] <= 14) {
			map[yPosition][xPosition] = map[yPosition][xPosition] + 91;
		}
		// If the traffic light is currently on and there is a vehicle on the
		// tile.
		// !WARNING! THIS DESTROYS THE VEHICLE - the user should reset the model
		// each time rather than pressing start again if the number of vehicles
		// is
		// important.
		...
		// If the light is already on and there is no vehicle on the tile it
		// will do nothing.
	}
}
