...
/**
 * Stores large data sets used by the application. Includes map/road network
 * data and statistical results of the model. This class is a singleton to allow
 * easy access by multiple classes.
 * 
 * @author Daniel Bane
 *
 */
public class Storage {
	...
	/**
	 * Stores the current state of the model (changes constantly as the model is
	 * ran). Can be reset by the user to contain initialMap.
	 */
	private double[][] temporaryMap;

	/**
	 * Is set to true when reset map is called. Tells the model vehicle movement
	 * loop to cancel.
	 */
	...
	/**
	 * Stores whether the data is currently being altered (the model is
	 * running).
	 */
	...
	/**
	 * Stores the average number of tiles each vehicle is from the vehicle in
	 * front.
	 */
	...
	public BufferedImage getDataAsImage() {
		...
		if (temporaryMap != null) {
			// Copies the data into a 1D array to allow the creation of an
			// Image.
			...
			for (int i = 0; i < pixels.length; i++) {
				...
				// Sets the colour of road pixels(which should be valued 1,2,3
				// or 4 depending on their N/E/S/W facing direction) to black.
				...
				// Sets the colour of vehicles(which should be valued the value
				// of the road tile they are on + 9) to blue.
				// i.e a north facing road (which has a value of 1) would have
				// the value of 10 if a vehicle was on it.
				...
				// Sets the colour of the traffic lights(which should be valued
				// 100) and the two adjacent tiles(which are valued as 200) to
				// red.
				else if (value > 100) {
					...
					if (!invertColours) {
						...
						// If invert colours is true it they will be set to
						// black.
						color = new Color(255, 255, 255);
					} else {
						color = new Color(0, 0, 0);
			...
			MemoryImageSource memSource = new MemoryImageSource(
					temporaryMap[0].length, temporaryMap.length, pixels, 0,
					temporaryMap[0].length);
	...
	/**
	 * Moves the data from temporaryMap into a 1D array - allows it to be turned
	 * into an image.
	 * 
	 * @return A 1D array.
	 */
	private double[] get1DArray() {
		...
		double[] tempArray = new double[temporaryMap.length
				* temporaryMap[0].length];
		// Cycles through each element in the 2D temporaryMap array and copies
		// it to tempArray.
		for (int i = 0; i < temporaryMap.length; i++) {
			for (int j = 0; j < temporaryMap[0].length; j++) {
				tempArray[j
						+ (i * (temporaryMap[0].length))] = temporaryMap[i][j];
	...
	/**
	 * This method turns an image into a buffered image. Code was obtained from
	 *
	 * http://www.rgagnon.com/javadetails/java-0601.html
	 */
	...
	/**
	 * Sets whether the stored data is undergoing alteration (the model is
	 * running).
	 */
	...
	/**
	 * Stores the average distance between each vehicle and the vehicle in
	 * front.
	 * 
	 * @param value
	 */
	...
	/**
	 * Sets the overall average percentage of vehicles that were stood still in the model.
	 * @param input The percentage.
	 */
	...
	/**
	 * Sets the overall average percentage of vehicles that were stood still in the model.
	 * @return The percentage average value.
	 */
	...
	/**
	 * Sets the overall average distance that was between each vehicle and the vehicle in front.
	 * @param input Percentage value.
	 */
	...
	/**
	 * Gets the overall average distance that was between each vehicle and the vehicle in front.
	 * @return The percentage average value.
	 */
